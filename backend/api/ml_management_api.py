...
class TrainingRequest(BaseModel):
    ...
    # Class Balancing - Focal Loss + Oversampling (NO class weights - they cause double compensation!)
    use_focal_loss: bool = Field(default=True, description="Use Focal Loss for hard example focus")
    focal_gamma: float = Field(default=1.5, ge=0, le=5, description="Focal Loss gamma (1.5 optimal with oversampling)")
    use_class_weights: bool = Field(default=False, description="DISABLED: oversampling already balances data")
    use_oversampling: bool = Field(default=True, description="Use oversampling (primary balancing method)")
    oversample_ratio: float = Field(default=1.0, ge=0, le=1, description="Oversample ratio (1.0 = full balance)")
...
async def start_training(
    ...
    # Сохраняем ссылку на основной event loop для WebSocket broadcasts
    ...
    def run_training_sync():
        ...
        try:
            loop.run_until_complete(
                _run_training_job(job_id=job_id, request=request, main_loop=main_loop)
            )
        finally:
            loop.close()
...
async def _run_training_job(job_id: str, request: TrainingRequest, main_loop: asyncio.AbstractEventLoop = None):
    """
    ...
    Args:
        job_id: ID job'а
        request: Параметры обучения
        main_loop: Main event loop для WebSocket broadcasts (из FastAPI)
    """
    global current_training_job
    ...
    try:
        ...
        # WebSocket broadcast: training completed (через main event loop)
        try:
            ws_manager = get_websocket_manager()
        ...
        # WebSocket broadcast: training failed (через main event loop)
        try:
            ws_manager = get_websocket_manager()
            broadcast_to_main_loop(
                ws_manager.broadcast_training_progress(
...
async def list_models(
    ...
    """
    Список всех моделей (только физически присутствующие на диске)
    ...
    try:
        ...
        # Get all models from local registry (only physical files)
        models = []
